...
export default class Scenario {
    ...
    async getStatsAutoRu(type){
        let response = await fetch('http://192.168.254.91/api/avito?type=' + type, {
            method: 'GET',
            headers: {
    ...
    async existsInnerId(inner_id, url, resource_id){
      //  let response = await fetch('http://192.168.254.91/api/announcement?filter[inner_id]=' + inner_id + '&filter[url]=' + url + '&filter[resource_id]=' + resource_id, {
        let response = await fetch('http://192.168.254.91/api/announcement?filter[inner_id]=' + inner_id + '&filter[resource_id]=' + resource_id, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json;charset=utf-8',
                'Accept': 'application/json;charset=utf-8'
    ...
    async sendUpdateToLog(inner_id, name, description, price, mileage, year, vin){
        let response = await fetch('http://192.168.254.91/api/updater?inner_id='+inner_id
            +'&name='+name+'&description='+description+'&price='+price+'&mileage='+mileage+'&year='+year+'&vin='+vin,
            {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json;charset=utf-8',
