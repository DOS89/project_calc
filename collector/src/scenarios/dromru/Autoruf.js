...
export default class Autoruf extends Scenario {
    ...
    async run(){
        ...
        let ura = await this.getStatsAutoRu('parse');
        console.log(JSON.parse(ura));
        ura = JSON.parse(ura);
        ...
        //url_go = 'https://auto.ru/cars/used/?sort=proven_owner-desc&top_days=9&page='+now.getMinutes();
        ...
        for (let i = 0; i < 1; i++) {
            console.log(now.getMinutes());
            console.log('sa');
            //links[i].href =  ura.url;
            console.log('ssa');
            console.log( ura.url);
            console.log( links);
            console.log('s');
            // todo: получаем ID объявления
            let link_array = ura.url.split('/');
            ...
            await PAGE.goto(ura.url, { waitUntil: "domcontentloaded" });
            await PAGE.waitForTimeout(5000);
            ...
            ura = await this.getStatsAutoRu('parse');
            console.log(JSON.parse(ura));
            ura = JSON.parse(ura);
        }
