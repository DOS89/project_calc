...
export default class Dromru7 extends Scenario {
    ...
    async run(){
        ...
        if(now.getMinutes()>=0){
            url_go = 'https://auto.drom.ru/used/all/?isOwnerSells=1';
        }
        if(now.getMinutes()>=3){
            url_go = 'https://auto.drom.ru/used/all/page8/?isOwnerSells=1';
        }
        if(now.getMinutes()>=5){
            url_go = 'https://auto.drom.ru/used/all/?cid[]=5026&cid[]=286&cid[]=301&cid[]=1294&cid[]=308&cid[]=309&cid[]=2246&cid[]=2241&cid[]=3913&cid[]=3905&cid[]=1303&cid[]=347&cid[]=3906&cid[]=1385&cid[]=357&cid[]=396&cid[]=2237&cid[]=3915&cid[]=527&cid[]=4386&cid[]=2730&cid[]=2272&cid[]=157&cid[]=3980&cid[]=3979&cid[]=320&cid[]=2372&cid[]=3996&cid[]=313&cid[]=3981&cid[]=3988&cid[]=3982&cid[]=393&cid[]=395&cid[]=2369&isOwnerSells=1';
        }
        if(now.getMinutes()>=7){
            url_go = 'https://surgut.drom.ru/auto/used/all/?isOwnerSells=1';
        }
        if(now.getMinutes()>=10){
            url_go = 'https://auto.drom.ru/used/all/?vinreport=1';
        }
        if(now.getMinutes()>=13){
            url_go = 'https://moscow.drom.ru/auto/used/all/page3/?distance=1000';
        }

        if(now.getMinutes()>=16){
            url_go = 'https://auto.drom.ru/region66/used/all/?distance=1000';
        }
        ...
        if(now.getMinutes()>=23){
            url_go = 'https://auto.drom.ru/region74/used/all/';
        }
        if(now.getMinutes()>=25){
            url_go = 'https://auto.drom.ru/used/all/?order=mileage&order_d=desc';
        }
        if(now.getMinutes()>=27){
            url_go = 'https://auto.drom.ru/used/all/?order=mileage';
        }
        if(now.getMinutes()>=30){
            url_go = 'https://auto.drom.ru/region24/used/all/?distance=1000';
        }

        if(now.getMinutes()>=34){
            url_go = 'https://auto.drom.ru/toyota/used/all/?owner_type=1&order=mileage&order_d=desc';
        }

        if(now.getMinutes()>=38){
            url_go = 'https://auto.drom.ru/region89/used/all/?distance=1000';
        }

        if(now.getMinutes()>=42){
            url_go = 'https://auto.drom.ru/region20/used/all/';
        }
        if(now.getMinutes()>=46){
            url_go = 'https://auto.drom.ru/region20/used/all/?distance=1000';
        }
        if(now.getMinutes()>=52){
            url_go = 'https://chelyabinsk.drom.ru/auto/used/all/';
        }
        if(now.getMinutes()>=56){
            url_go = 'https://abakan.drom.ru/auto/used/all/';
        }
        ...
        for (let i = 0; i < 20; i++) {
            ...
            let car = await PAGE.evaluate(() => {
                ...
                if(c.name === undefined){
                    console.log("Проверяем закрытое объявление");
                    c.close = 1;
                    try {
                        c.name = document.querySelector('h1 .css-1qkyk94')?.innerText;
                        console.log(c.name);
                    } catch (e) {
                ...
                if( c.close == 1){
                    c.year = document.querySelector('h1 .css-1qkyk94')?.innerText;
                } else {
                ...
                allnames.forEach((element) => {
                    ...
                    if(element){
                        ...
                        if(element?.innerHTML.indexOf('Комплектация') != -1){
                            console.log('Проверка 16');
                            try {
                                c.comp = props[pos]?.innerText;
                                c.comphref = props[pos]?.innerHTML;
                                c.comphref = props[pos]?.split('"')[1];
                                console.log(c.comphref);
                            }catch (e){}
