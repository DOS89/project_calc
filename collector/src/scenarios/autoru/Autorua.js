...
export default class Autorua extends Scenario {
    ...
    async run(){
        ...
        let url_go = 'https://auto.ru/sankt-peterburg/cars/used/';
        var now = new Date();
        console.log(now.getMinutes());
        if(now.getMinutes()>=0){
            url_go = 'https://auto.ru/sankt-peterburg/cars/used/';
        }
        if(now.getMinutes()>=3){
            url_go = 'https://auto.ru/moskva/cars/used/';
        }
        if(now.getMinutes()>=5){
            url_go = 'https://auto.ru/kazan/cars/used/?top_days=2';
        }
        if(now.getMinutes()>=7){
            url_go = 'https://auto.ru/vladivostok/cars/used/?top_days=2';
        }
        if(now.getMinutes()>=10){
            url_go = 'https://auto.ru/sochi/cars/used/?top_days=2';
        }
        if(now.getMinutes()>=13){
            url_go = 'https://auto.ru/sochi/cars/vaz/used/?top_days=2';
        }
        if(now.getMinutes()>=16){
            url_go = 'https://auto.ru/habarovsk/cars/used/?top_days=2';
        }
        if(now.getMinutes()>=19){
            url_go = 'https://auto.ru/novosibirskaya_oblast/cars/vaz/used/?top_days=2';
        }
        if(now.getMinutes()>=23){
            url_go = 'https://auto.drom.ru/region2/used/all/';
        }
        if(now.getMinutes()>=30){
            url_go = 'https://auto.drom.ru/region76/used/all/';
        }

        if(now.getMinutes()>=34){
            url_go = 'https://auto.ru/krasnodarskiy_kray/cars/used/?top_days=2';
        }
        if(now.getMinutes()>=38){
            url_go = 'https://auto.ru/moskva/cars/used/';
        }
        if(now.getMinutes()>=42){
            url_go = 'https://auto.ru/moskva/cars/volkswagen/used/?top_days=2';
        }
        if(now.getMinutes()>=46){
            url_go = 'https://auto.ru/moskva/cars/used/';
        }
        if(now.getMinutes()>=52){
            url_go = 'https://auto.ru/cars/vendor-domestic/used/?top_days=2';
        }
        if(now.getMinutes()>=56){
            url_go = 'https://auto.ru/moskva/cars/used/';
        }
        //url_go = 'https://auto.ru/cars/used/?top_days=31&page=' + (100 - now.getMinutes());
        console.log(url_go);
        await PAGE.goto(url_go, { referer: "https://auto.ru/cars/used/", waitUntil: "domcontentloaded" });
        await PAGE.waitForTimeout(2000);
        await PAGE.waitForTimeout(2000);
        ...
        for (let i = 0; i < 10; i++) {
            ...
            let dar = await PAGE.evaluate(() => {
                let result = [];
                let groups = document.querySelectorAll('.ComplectationGroupsDesktop__group');
                for (let i = 0; i < groups.length; i++) {
                    let category = groups[i].querySelector('.ComplectationGroupsDesktop__groupTitle');
                    let categoryName = category?.innerText;
                    if (categoryName !== '' && categoryName !== null) {
                        result[i] = [categoryName];
                        let lis = groups[i].querySelectorAll('.ComplectationGroupsDesktop__itemList li');
                        for (let q = 0; q < lis.length; q++) {
                            let spans = lis[q].querySelectorAll('span');
                            let innerText = spans[1]?.innerText;
                            if (innerText !== '') {
                                result[i].push(innerText);
                            }
                        }
                    }
                }
            ...
            let car = await PAGE.evaluate(() => {
                ...
                if(c.generation === '') c.generation = 'Не найдена';
                ...
                c.body =  document.querySelector('.CardInfoRow_bodytype a')?.innerText;
                c.bodytype =  document.querySelector('.CardInfoRow_bodytype a')?.innerText;
                c.color = lastChildText('.CardInfoRow_color .CardInfoRow__cell a');
                ...
                c.advantage = document.querySelector('.ReviewFeatureCounts__feature_positive')?.innerText;
                if(c.advantage == 'undefined'){
                    c.advantage = 0;
                }
                c.flaw = document.querySelector('.ReviewFeatureCounts__feature_negative')?.innerText;
                if(c.flaw == 'undefined'){
                    c.flaw = 0;
                }
            ...
            if(car.description === undefined){
                car.description = 'отсутсвует';
            } else {
                car.description = car.description.replace(/\r?\n/g, "");
            }
           ...
           //Комплектации со страницы
            car.complectation = [];
            car.complectation = dar;
